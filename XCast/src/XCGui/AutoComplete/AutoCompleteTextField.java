...
public class AutoCompleteTextField extends JTextField implements AutoComplete {
    ...
    addKeyListener( new KeyAdapter() {
      ...
      public void keyReleased(KeyEvent e) {
        ...
        if (charCodePressed == KeyEvent.VK_DELETE ||
            charPressed == KeyEvent.CHAR_UNDEFINED || charCodePressed == KeyEvent.VK_ENTER) {
          return;
        }
        ...
        String split[] = split(input, new char[]{':', '@', '/', '[', ']', '(', ')', ' ', '=', '<', '>', '!'});
        String result = "";
      	//System.out.println(split[1]);
        if((result = lookup(split[1])) != null) {
          setText(split[0]+result);
  ...
  protected String[] split(String s, char[] ch) {
  	int max = -1;
  	for (int i=0; i<ch.length; i++) {
  		int current = s.lastIndexOf(ch[i]);
  		max = ((current > max) ? current : max);
  	}
  	return (max > -1) ? new String[]{s.substring(0, max+1),s.substring(max+1, s.length())} : new String[]{"", s}; 
  }
