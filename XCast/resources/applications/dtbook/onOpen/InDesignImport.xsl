...
* Tabeller hanteras inte fullständigt. Problem kan uppstå med vissa colspan- och rowspankombinationer.
* Bilder hanteras inte fullständigt. Vissa bilder försvinner i sorteringen.
-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:aid="http://ns.adobe.com/AdobeInDesign/4.0/" exclude-result-prefixes="aid">
	...
	<xsl:template match="*[parent::Artikel and not(self::page or self::Figure)]">
		<xsl:choose>
	...
	<xsl:template match="*[@href_fmt and count(descendant::text())=0]">
		<img src="{@href_fmt}"/>
	</xsl:template>
